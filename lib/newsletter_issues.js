...
import { issueUnsubUrlFor, normalizeEmail } from './newsletter.js';
...
// Owner manual add: create (or confirm) an ACTIVE, confirmed subscriber.
// Reuses normalizeEmail. NEVER reactivates a suppressed (bounced/complained)
// OR an unsubscribed row — both honor a recorded opt-out and protect sender
// reputation (an 'unsubscribed' status can be the SUBSCRIBER's own opt-out, not
// just an owner removal, and the two are indistinguishable; refusing it here
// keeps this consistent with importResendAudience, which also never reactivates
// unsubscribed). A brand-new address is inserted active/source='manual'; a
// never-confirmed 'pending' row is confirmed by this explicit owner action; an
// already active address is a no-op. To rejoin, a suppressed/unsubscribed person
// must re-subscribe through the public double-opt-in form. Returns
// { subscriber, result }.
export async function addSubscriberManual(email, { now } = {}) {
  const normalized = normalizeEmail(email);
  if (!normalized) throw new IssueError('invalid_email');
  const supabase = svc();
  const { data: existing, error: selErr } = await supabase
    .from(SUBS).select('id, status').eq('email', normalized).maybeSingle();
  if (selErr) throw new IssueError('db_error', selErr.message);
  const ts = nowIso(now);

  if (!existing) {
    const { data, error } = await supabase
      .from(SUBS)
      .insert({ email: normalized, status: 'active', source: 'manual', confirmed_at: ts })
      .select('id, email, status').maybeSingle();
    if (error) throw new IssueError('db_error', error.message);
    return { subscriber: data, result: 'added' };
  }
  if (SUPPRESSED.has(existing.status)) throw new IssueError('suppressed', `status=${existing.status}`);
  if (existing.status === 'unsubscribed') throw new IssueError('unsubscribed', 'status=unsubscribed');
  if (existing.status === 'active') {
    return { subscriber: { id: existing.id, email: normalized, status: 'active' }, result: 'already_active' };
  }
  const { data, error } = await supabase
    .from(SUBS)
    .update({ status: 'active', confirmed_at: ts, unsubscribed_at: null })
    .eq('id', existing.id).select('id, email, status').maybeSingle();
  if (error) throw new IssueError('db_error', error.message);
  return { subscriber: data, result: 'activated' };
}
