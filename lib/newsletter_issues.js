...
export async function runSendBatch({ limit = SEND_BATCH_LIMIT, sendFn = sendEmail, now } = {}) {
  ...
  for (const row of queued) {
    ...
    try {
      const res = await sendFn({ to: sub.email, subject: email.subject, html: email.html, text: email.text, headers: email.headers });
...
export async function runNewsletterCron({
  ...
  try {
    ...
    await releaseLock(supabase, holder, {
      success: true, synced: outcome.processed || 0, created: outcome.sent || 0, updated: outcome.failed || 0,
    });
    return outcome;
  } catch (err) {
    const detail = String(err?.detail || err?.message || err).slice(0, 1000);
    const code = err?.code || 'send_failed';
    await releaseLock(supabase, holder, { success: false, error: `${code}: ${detail}` });
...
export async function sendNow({
  ...
  try {
    ...
    const batch = await runSendBatch({ limit, sendFn, now });
    await releaseLock(supabase, holder, {
      success: true, synced: batch.processed || 0, created: batch.sent || 0, updated: batch.failed || 0,
    });
