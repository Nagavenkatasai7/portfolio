...
//
// The raw secret is NEVER used directly as the salt: it is run through a
// purpose-bound HMAC label ('analytics-ip-salt-v1') so this analytics salt is
// cryptographically decoupled from the session-signing secret (and from any
// other consumer of the same env var) — a leak of this derived value can't be
// correlated back to the secret. This does NOT touch lib/auth/session.js's own
// key derivation. Bumping the label one-time rebuckets existing IP hashes, which
// is acceptable for analytics-only data.
function salt() {
  const secret = process.env.SESSION_SECRET || process.env.SUPABASE_SERVICE_ROLE_KEY;
  // Unconfigured (local/dev only — no real reader IPs to protect): return a
  // plainly non-secret constant rather than HMAC-ing with a hardcoded fallback
  // key (which is both weak and a hardcoded-secret smell). In production
  // SESSION_SECRET — or the service-role key — is always set, so the HMAC path
  // below always runs and behavior is unchanged.
  if (!secret) return 'analytics-unconfigured-salt';
  return createHmac('sha256', secret).update('analytics-ip-salt-v1').digest('hex');
}
