...
//
// Hardening (defense-in-depth vs. CSV formula injection at signup): the local
// part's FIRST character may not be a spreadsheet-formula lead (= + - @) or any
// ASCII control char, so an address like `=HYPERLINK("http://evil")@x.co` is
// rejected here — the authoritative escape is still output-side in
// subscribersToCsv. Mid-string '+' (plus-addressing, e.g. user+tag@x.com) is
// deliberately still allowed; only a LEADING + is rejected.
const EMAIL_RE = /^[^\s@=+\-\x00-\x1f\x7f][^\s@\x00-\x1f\x7f]*@[^\s@.]+(?:\.[^\s@.]+)+$/;
...
async function safeSend(sendFn, args) {
  try {
    ...
    // Include e.detail (the provider's response body, e.g. Resend's validation
    // error) — a bare status like "resend_send_failed_422" is undiagnosable
    // from logs alone.
    console.error(
      '[newsletter] confirm email send failed (row persisted, re-sendable):',
      e?.message || e,
      e?.detail || '',
    );
  }
...
export async function confirmToken(rawToken) {
  ...
  // Permanent suppression (a prior hard bounce / spam complaint): a stale
  // confirm link must NOT re-arm the address (which would resurrect a
  // reputation-damaging sender to a dead/hostile mailbox). No state change;
  // answer exactly like a dead link so nothing is leaked or re-subscribed.
  if (SUPPRESSED.has(row.status)) return { ok: false, error: 'invalid' };
...
// -------------------------------------------------------------------------
// issueUnsubUrlFor(subscriberId) — mint a FRESH unsubscribe token for one
// subscriber, persist its hash, and return the ready-to-embed unsub link (Phase
// N2). Every issue email a recipient receives carries THEIR OWN working opt-out
// link. Because only the hash is ever stored (see the module header), the raw
// token needed to build a link does not survive past send time, so it MUST be
// re-minted here — this rotates the subscriber's unsub token on each send (the
// same rotation the confirm-email flow performs). Documented tradeoff: a link
// from a PRIOR email stops working once a newer email is sent; the newest email
// (and the admin "Remove") always work. Returns the full URL, or null if the DB
// is unconfigured or the update failed (the caller treats a null as "no link").
// -------------------------------------------------------------------------
export async function issueUnsubUrlFor(subscriberId) {
  ...
  const { error } = await supabase
    .from(TABLE)
    .update({ unsub_token_hash: unsub.hash })
    .eq('id', subscriberId);
  if (error) return null;
  return unsubUrlFor(unsub.raw);
}
...
export async function unsubscribeToken(rawToken) {
  ...
  // Never LAUNDER a permanently-suppressed row (bounced/complained) down to
  // 'unsubscribed' — that state is re-subscribable, so it would let a
  // suppressed address quietly re-arm itself via the public subscribe form.
  // The .neq guards make the update an atomic no-op for those rows (no
  // read-modify-write race), while a normal active/pending row still opts out.
  await supabase
    .from(TABLE)
