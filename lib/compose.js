...
export function buildComposerItem(input, seed = Date.now()) {
  ...
  // Publishing "now" with a FUTURE published_at is a trap: the public /blog has
  // no date gate, so the post would appear immediately AND sort pinned to the top
  // forever. Reject it — the owner should clear the date, or save it as a draft.
  // A 60s grace window absorbs browser/server clock skew.
  if (status === 'published' && new Date(published_at).getTime() > Date.now() + 60_000) {
    throw new ComposeError('future_publish_date');
  }
