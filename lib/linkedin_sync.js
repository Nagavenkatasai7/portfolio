...
export async function runLinkedinSync(opts = {}) {
  ...
  try {
    ...
    logger.log(`[linkedin-sync] ok — synced=${synced} created=${created} updated=${updated}`);
    return { ok: true, synced, created, updated };
  } catch (err) {
...
//   'stale'   — no successful run in over ~a day (cron is every 6h)
...
export function syncStaleness(state, { now = Date.now } = {}) {
  ...
  const level = state.last_error ? 'error' : ageMs > 26 * 3600 * 1000 ? 'stale' : 'ok';
  return { level, ageMs };
}
